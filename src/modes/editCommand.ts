    ...
    async () => {
      ...
      if (!config.model) {
        vscode.window.showWarningMessage(
          'No model configured for Chat mode. Please configure ollamaCopilot.chatMode.model in settings.',
          'Open Settings'
        ).then(choice => {
          if (choice === 'Open Settings') {
            vscode.commands.executeCommand('workbench.action.openSettings', 'ollamaCopilot.chatMode');
          }
        });
        return;
      }
